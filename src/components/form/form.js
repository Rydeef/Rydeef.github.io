...
const renderFieldStats = ({
  input,
  type
}) => (
  <input {...input} maxLength="3" onChange={e => input.onChange(replacerStats(e))} type={type} />
)
    ...
    <form onSubmit={handleSubmit(submit)}>
      {stats.map((stat, index) => (
        <div className="filter-stats-main" key={index}>
          ...
          <Field name={`${stat.replace(" ", "")}From`} component={renderFieldStats}  type="text" />
          -
          <Field name={`${stat.replace(" ", "")}To`} component={renderFieldStats} type="text" />
        </div>
      ))}
...
export default connect(state => {
  const attack = { from: Number(selector(state, "AttackFrom")), to: Number(selector(state, "AttackTo")) }
